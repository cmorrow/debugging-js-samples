...
const AuthenticationForm = (function () {
  ...
  var Module = {
    init: function (options) {
      ...
      $("#authenticationForm").on("submit", this.submitHandler);
    },

    teardown: function () {
      // detach event handlers so that subsequent test runs do not interfere
      // with each other.
      $("#authenticationForm").off("submit", this.submitHandler);
    },
  ...
  function submitForm(done) {
    ...
    if (username && password) {
      checkAuthentication.call(this, username, password, function (
        error,
        user
      ) {
        if (error) {
          $("#username_password_required").hide();
